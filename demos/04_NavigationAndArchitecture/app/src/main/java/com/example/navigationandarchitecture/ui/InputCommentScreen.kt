...
@Composable
fun InputCommentScreen (commentViewModel:CommentViewModel, navController:NavHostController, modifier:Modifier = Modifier) {
                ...
                Button(onClick = {
                    ...
                    if (comment == "") {
                        ...
                        Toast.makeText(
                            ...
                            .show()//show the message
                        */
                        val newMessageRow = MessageRow(R.drawable.profile, comment)
                        commentViewModel.addComment(newMessageRow)//update the UI state
                        comment = ""
...
//Update the methods below to display the commentList, which is now a List<MessageRow> instead of List<String>
...
@Composable
fun CommentList (commentList:List<MessageRow>, navController:NavHostController, modifier:Modifier = Modifier){
    LazyColumn {
        ...
        itemsIndexed(commentList) { index, item ->
            CommentRow(navController = navController, message = MessageRow(R.drawable.profile,item.comment), index = index, modifier = modifier.padding(6.dp))
        }
    }
}
